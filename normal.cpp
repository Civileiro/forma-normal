...
class TabelaVerdade {
  public:
	TabelaVerdade(std::string_view formula) : formula {formula} {
  ...
  private:
	std::string formula;
	std::vector<std::tuple<std::map<char, bool>, bool>> tabela;
...
void imprimirTabela(const std::vector<std::tuple<std::map<char, bool>, bool>> &tabela, std::string_view formula) {
	...
	for (auto &[variaveis, resultado] : tabela) {
		for (auto &[nome, valor] : variaveis) {
			std::cout << "| " << (valor ? 'V' : 'F') << ' ';
		}
		const auto espacos = std::string(formula.length() / 2, ' ');
		std::cout << "|" << espacos << (resultado ? 'V' : 'F') << espacos << (formula.length() % 2 ? "  |\n" : " |\n");
	}
	std::cout << separador.str();
}

int main() {
	// pegar a formula do usuario
	std::string formula;
	std::cout << "Digite a formula: ";
	std::getline(std::cin, formula);

	try {
		TabelaVerdade tb {formula};				 // iniciar o objeto com a formula
		imprimirTabela(tb.getTabela(), formula); // imprimir resultado
	} catch (std::runtime_error &e) {
		// caso a formula esteja invalida ou ocorra um outro erro, ele serah imprimido
		std::cout << e.what();
	}
}